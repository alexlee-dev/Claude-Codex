...
export function renderBanner(options: BannerOptions): string {
  const title = options.title ?? 'Claude Codex'
  const width = Math.max(
    MIN_BANNER_WIDTH,
    Math.min(process.stdout.columns ?? DEFAULT_BANNER_WIDTH, MAX_BANNER_WIDTH),
  )
  const innerWidth = width - 2
  const rightWidth = innerWidth - INNER_PADDING * 2 - CLAWD_WIDTH - CLAW_GAP
  ...
  const infoLines = [
    ...
    `${dim('mode')} ${truncate(options.mode, rightWidth - 5)}`,
  ]

  const rows = [
    buildTopBorder(width, title),
    buildRow(width),
